...
class JFormFieldText extends JFormField
{
	...
	protected function getInput()
	{
        ...
        $translate    = $this->element['translate'] == 'true' ? true : false;;
        ...
        if($translate){
            $lamguages = language4UHelper::getLanguages();

            foreach($lamguages as $i => $language)
            {
                $html[] = '<div class="input-prepend">';
                    $html[] = '<span class="add-on">' . language4UHelper::getImage($language) . '</span>';
                    if($i == 0){
                        ...
                        $html[] = '<div style="display:none;">' . str_replace('jform_' . $name, 'lang_' . $language->lang_id . '_' . $name, $this->getLabel()) . '</div>';

                        $html[] = language4UHelper::getInput($table, $name, $language->lang_id, Input4U::getInt('id', 'REQUEST'), 'text', false, $this->class);
                    }
                $html[] = '</div>';

                if($i < count($lamguages) - 1){
                    $html[] = '&nbsp;&nbsp;&nbsp;';
                }
            }
        } else {
