 ...
 * @since       1.6
 */
class JFormFieldPicture extends JFormField
{
	/**
	 * The form field type.
	 *
	...
	protected function getInput()
	{
		...
		$html[] = '	<input type="text" class="input-small" name="picture['.$this->imagekey.']" id="' . $this->id . '"' . ' value="'
			. htmlspecialchars($this->value, ENT_COMPAT, 'UTF-8') . '"' . ' readonly="readonly"' . $attr . ' />';

		$directory = (string) $this->element['directory'];
		
		$objectId = Input4U::getInt('id');
		if($objectId>0)
		{
			$directory = $directory.'/'.$objectId;
			App4U::cdir(JPATH_ROOT . '/images/' . $directory);
		}
		if ($this->value && file_exists(JPATH_ROOT . '/' . $this->value))
		{
		...
		if ($this->element['disabled'] != true)
		{
			...
			$html[] = 'jInsertFieldValue(\'\', \'' . (int)$this->id . '\');';
			$html[] = 'return false;';
			$html[] = '">';
